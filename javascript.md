...
script 위치는 html 어디서나 가능
그래도 보통은 body 끝나기 전 바로 위에 위치

--------------------------------------------

<10월 29일 메모>

NaN은 무조건 뭘 하든 비교를 하면 false로 반환됨
...
var num = '2234.5678'; console.log(~~num); // 2234


break를 사용하면 나를 감싸고 있는 반복문 하나만 탈출
따라서 한 번에 묶어서 그 묶음을 탈출하고 싶을 땐 label을 사용하기도 한다.
(break label문 : 하나의 반복문을 빠져나가려는 break문과 달리 여러 개의 반복문을 탈출할 때 사용)
(label은 반드시 break와 같이 사용해야 한다.)


return은 종료되는 구문으로 사용


delete로 지워봤자 메모리에 남아있기 때문에 null로 설정을 해주는 편이 더 좋음
ex) delete human.age // 권하지 X
    human.age = null // 권함 O

--------------------------------------------

<11월 5일>
map: key와 value로 이루어진 자료형

DOM은 설계 도면이다.

자바스크립트 안에 DOM이 있는 것이 아닌
Document를 어떻게 표현할지에 관한 도면일 뿐이다.
즉, ECMAScript 안에 포함 X

DOM을 자바스크립트의 기능이라고 말하는 것은 잘못된 것인가요??
=>YES ,  브라우저의 기능이라고 봐야 한다.

window API: 실제 코드를 알지 못해도 불러 와서 사용할 수 있도록 해줌

DOM API
: 돔에 관한 상세 내용을 모르지만 document.querySelector와 같은 것을 사용해서 사용할 수 있도록 하는 것
  ex) 메뉴판 (api를 만드는건 메뉴판을 만드는 것)

DOM: 실제 HTML에 파싱되어서 트리 모양으로 만들어지는 것

NodeList 안에 담겨 있는 것들은 서로 타입이 다르다.

this는 앞에서 나를 호출한 애다! 라고 생각하면 됨
